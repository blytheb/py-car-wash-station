class Car:
    def __init__(self, comfort_class: int, mark: int, brand: str) -> None:
...
class CarWashStation:
    def __init__(self, dis: float, powr: int, rate: float, ct: int) -> None:
        self.distance_from_city_center = dis
        self.clean_power = powr
        self.average_rating = rate
        self.count_of_ratings = ct
...
to 1 decimal;"""
    ...
    """method that adds a single rate to the wash station, and based
      on this single rate `average_rating` and `count_of_ratings`
      should be changed:"""
